...
public class Peocedure_SelectAmountWithInOutCursor {
	public static void main(String[] args)  {
		...
		try {
			...
			/*call callableStatement --> syntex 
			 {selectWithInOutCursor(?,?)}
			 registerOutParameter(1, TYPE.NEWMERIC);
			 * */
			cs=con.prepareCall("{call  selectAmountWithInOutCursor(?,?)}");
			...
			cs.execute();
			//index shopuld be same what we kept at getObject()
