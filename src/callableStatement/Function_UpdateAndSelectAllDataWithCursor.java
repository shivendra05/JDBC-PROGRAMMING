...
public class Function_UpdateAndSelectAllDataWithCursor {
	public static void main(String[] args)  {
		...
		try {
			...
			/*call callableStatement --> syntex 
			 ...
			 cs.setInt(1, account);
			 * */
			cs=con.prepareCall("{?=call  fUpdateSelectAllDataWithCursor(?)}");
			...
			while(rs.next()) {
				System.out.println(rs.getInt(1)+" "+rs.getString(2)+" "+rs.getInt("AGE")+" "+rs.getString(4)+" "+rs.getString("CITY")+" "+rs.getInt("AMOUNT"));
			}
